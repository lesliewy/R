...
corrNotionData <- function(dfIndex, dfNotions, cormethod){
  print("this is in corrNotionData()")
  groupNotions <- groupNotionData(dfNotions)
  # 注意参数的传递
  cordata = lapply(groupNotions, calOneCor, dfIndex, cormethod)
  # 将list转成data.frame
  result = {}
  for(i in cordata){
...
groupNotionData <- function(dfNotions){
  print(paste("this is in groupNotionData()"))
...
calOneCor <- function(dfNotion, dfIndex, cormethod){
  #print(paste("this is in calOneCor()", cormethod))
  # 大盘和板块的日期必须一致
  ...
  cordata <- cor.test(dfnotion1$CHANGE_PCT, dfIndex1$CHANGE_PCT,method=cormethod)
